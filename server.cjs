...
// Новый маршрут для открытия мини-приложения
app.post('/api/open-mini-app', (req, res) => {
  console.log('Received request:', req.body);
  const { userId, timestamp } = req.body;

  const query = 'INSERT INTO user_activity (user_id, timestamp) VALUES (?, ?)';
  db.query(query, [userId, timestamp], (err, result) => {
    if (err) {
