...
const MineContent: React.FC<MineContentProps> = ({
  ...
  const [lastCollectTime, setLastCollectTime] = useState<number>(() => {
    const saved = localStorage.getItem('lastCollectTime');
  ...
  useEffect(() => {
    const updateData = () => {
      setEarnedCoins(calculateEarnedCoins());
      setTimer(calculateRemainingTime());
    };

    updateData();
    const interval = setInterval(updateData, 60000); // Обновляем каждую минуту
  ...
  const handleCollectCoins = async () => {
    ...
    try {
      ...
      localStorage.setItem('lastCollectTime', Date.now().toString());
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full pb-16"> {/* Добавили pb-16 */}
        ...
        {/* Новый дизайн для отображения заработанных монет и таймера */}
        <div className="px-4 mt-4">
        ...
        {/* Кнопка "Забрать" */}
        <div className="px-4 mt-4 mb-20"> {/* Добавили mb-20 для отступа от навигационной панели */}
          <button
