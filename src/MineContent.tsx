...
const MineContent: React.FC<MineContentProps> = ({
  ...
  const [selectedMineUpgrade, setSelectedMineUpgrade] = useState<string | null>(
    null
  );

  // Состояния для накопления монет
  const [lastCollectTime, setLastCollectTime] = useState<number>(Date.now());
  const [earnedCoins, setEarnedCoins] = useState<number>(0);
  const [timer, setTimer] = useState<number>(0);

  const MAX_ACCUMULATION_TIME = 3 * 60 * 60 * 1000; // 3 часа
  ...
  const calculateTotalIncome = (
    ...
    for (const [key, level] of Object.entries(upgrades)) {
      const upgradeLevel =
        upgradeLevels[key as keyof typeof upgradeLevels][level - 1];
  ...
  // Функция для получения данных накопления с сервера
  const fetchAccumulationData = async () => {
    if (userId === null) return;

    try {
      const response = await fetch(`/get-accumulation-data?userId=${userId}`);
      const data = await response.json();

      if (data) {
        const serverLastCollectTime = new Date(
          data.lastCollectTime
        ).getTime();
        setLastCollectTime(serverLastCollectTime);
        setEarnedCoins(data.earnedCoins || 0);
      } else {
        // Если нет данных, устанавливаем текущее время и нулевые накопленные монеты
  ...
  useEffect(() => {
    let accumulationInterval: number;

    const startAccumulation = () => {
      accumulationInterval = window.setInterval(() => {
        const currentTime = Date.now();
        const elapsedTime = currentTime - lastCollectTime;

        let accumulationTime = elapsedTime;
        if (accumulationTime >= MAX_ACCUMULATION_TIME) {
          accumulationTime = MAX_ACCUMULATION_TIME;
          clearInterval(accumulationInterval); // Останавливаем накопление после 3 часов
        }

        const earned = (incomePerHour / 3600) * (accumulationTime / 1000);

        setEarnedCoins(Math.floor(earned));

        const remainingTime = MAX_ACCUMULATION_TIME - accumulationTime;
        setTimer(remainingTime);
      }, 60000); // Обновляем каждую минуту
    };

    // Начинаем накопление только если incomePerHour > 0
    if (incomePerHour > 0) {
      startAccumulation();
    }
  ...
  const handleUpgrade = () => {
    if (selectedMineUpgrade) {
      ...
      if (currentLevel < 10) {
        ...
        const upgradeData =
          upgradeLevels[
            selectedMineUpgrade as keyof typeof upgradeLevels
          ][nextLevel - 1];
        ...
        if (points >= upgradeData.cost) {
          ...
          setNotificationMessage(
            `Улучшено ${selectedMineUpgrade} до уровня ${nextLevel}`
          );
          ...
          fetch('/save-data', {
            ...
            .then((data) => {
              if (data.success) {
                ...
                console.error(
                  'Ошибка на сервере: данные не были сохранены'
                );
              }
            })
            .catch((error) => {
  ...
  // Обновляем данные накопления на сервере при выходе со страницы
  useEffect(() => {
    return () => {
      if (userId !== null) {
        fetch('/update-accumulation-data', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  }, [userId, lastCollectTime, earnedCoins]);

  return (
    <>
      {notificationMessage && (
        <UpgradeNotification message={notificationMessage} />
      )}
      <div className="flex flex-col h-full pb-16">
          ...
          >
            ...
            >
              ...
              <p className="text-center text-gray-400 mb-4">
                ...
                Описание улучшения. Это улучшение поможет вам увеличить
                производительность и заработать больше монет.
              </p>
                ...
                <>
                  ...
                  >
                    Улучшить (
                    {
                      upgradeLevels[
                        selectedMineUpgrade as keyof typeof upgradeLevels
                      ][upgrades[selectedMineUpgrade]].cost
                    }{' '}
                    монет)
                  </button>
                </>
        ...
        <div className="mt-auto px-4 mb-20">
